...
Networking::Networking(QWidget* parent, bool show_advanced) : QWidget(parent), show_advanced(show_advanced){
  ...
  QLabel* warning = new QLabel("مدير الشبكة معطل!");
...
void Networking::attemptInitialization(){
  ...
  if (show_advanced) {
    QPushButton* advancedSettings = new QPushButton("خيارات اضافية");
...
void Networking::connectToNetwork(Network n) {
  if (n.security_type == SecurityType::OPEN) {
    ...
    QString pass = InputDialog::getText(n.ssid + "من فضلك أدخل رقمك السري", 8);
    wifi->connect(n, pass);
  }
}
...
void Networking::wrongPassword(QString ssid) {
  for (Network n : wifi->seen_networks) {
    if (n.ssid == ssid) {
      QString pass = InputDialog::getText(n.ssid + " كلمة مرور خاطئة", 8);
      wifi->connect(n, pass);
      return;
    }
  }
...
AdvancedNetworking::AdvancedNetworking(QWidget* parent, WifiManager* wifi): QWidget(parent), wifi(wifi){
  ...
  // Back button
  QPushButton* back = new QPushButton("عد");
  back->setFixedSize(500, 100);
  connect(back, &QPushButton::released, [=](){emit backPress();});
  vlayout->addWidget(back, 0, Qt::AlignLeft);

  // Enable tethering layout
  ToggleControl *tetheringToggle = new ToggleControl("تمكين الربط", "", "", wifi->tetheringEnabled());
  ...
  editPasswordButton = new ButtonControl("ربط كلمة المرور", "تغير", "", [=](){
  ...
  // IP address
  ipLabel = new LabelControl("IP عنوان", wifi->ipv4_address);
...
WifiUI::WifiUI(QWidget *parent, WifiManager* wifi) : QWidget(parent), wifi(wifi) {
  ...
  // Scan on startup
  QLabel *scanning = new QLabel("البحث عن شبكة");
...
void WifiUI::refresh() {
  ...
  for (Network &network : wifi->seen_networks) {
    ...
    // connect button
    QPushButton* btn = new QPushButton(network.security_type == SecurityType::UNSUPPORTED ? "غير قادر على الاتصال" : (network.connected == ConnectedType::CONNECTED ? "متصل" : (network.connected == ConnectedType::CONNECTING ? "연결중" : "연결")));
