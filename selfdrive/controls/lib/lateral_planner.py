...
class LateralPlanner():
  ...
  def update(self, sm, CP):
    ...
    # use laneless, it might mitigate abrubt steering at stopping?
    elif sm['radarState'].leadOne.dRel < 25 and sm['radarState'].leadOne.vRel < 0 and (abs(sm['controlsState'].steeringAngleDesiredDeg) - abs(sm['carState'].steeringAngleDeg)) > 2.5 and self.lane_change_state == LaneChangeState.off:
      ...
      self.laneless_mode_status = False
      self.laneless_mode_at_stopping = False
    elif self.laneless_mode_at_stopping and (v_ego < 5 or not self.laneless_mode_at_stopping_timer):
      d_path_xyz = self.path_xyz
      path_cost = np.clip(abs(self.path_xyz[0,1]/self.path_xyz_stds[0,1]), 0.5, 5.0) * MPC_COST_LAT.PATH
      # Heading cost is useful at low speed, otherwise end of plan can be off-heading
      heading_cost = interp(v_ego, [5.0, 10.0], [MPC_COST_LAT.HEADING, 0.0])
      ...
      self.libmpc.set_weights(path_cost, heading_cost, CP.steerRateCost)
      self.laneless_mode_status = True
    elif self.laneless_mode == 2 and (self.LP.lll_prob < 0.1 or self.LP.rll_prob < 0.1) and self.lane_change_state == LaneChangeState.off:
      d_path_xyz = self.path_xyz
      path_cost = np.clip(abs(self.path_xyz[0,1]/self.path_xyz_stds[0,1]), 0.5, 5.0) * MPC_COST_LAT.PATH
      # Heading cost is useful at low speed, otherwise end of plan can be off-heading
      heading_cost = interp(v_ego, [5.0, 10.0], [MPC_COST_LAT.HEADING, 0.0])
    ...
    elif self.laneless_mode == 2 and (self.LP.lll_prob > 0.3 and self.LP.rll_prob > 0.3) and self.laneless_mode_status_buffer and not self.laneless_mode_at_stopping and self.lane_change_state == LaneChangeState.off:
      d_path_xyz = self.LP.get_d_path(v_ego, self.t_idxs, self.path_xyz)
      self.libmpc.set_weights(MPC_COST_LAT.PATH, MPC_COST_LAT.HEADING, CP.steerRateCost)
      self.laneless_mode_status = False
      self.laneless_mode_status_buffer = False
